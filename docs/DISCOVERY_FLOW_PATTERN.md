# Target discovery flow coverage-gap pattern
...
The current `CLMDiscoveryFlow 1.1.0.1` implements credential, application-owner, source-environment, coverage-gap, and renewal-event writes, but it does **not** yet create or complete `clm_discoveryrun` records. Treat the run-record steps below as the required next revision rather than current behaviour.
           ...
           For each object:
               ...
               on failure:
                   Upsert clm_coveragegap by externalId = '{scopeType}|{objectId}'
                       ...
                       resolutionHint  = buildHint(scopeType, gapType, discoveryPrincipal)
...
## `buildHint(scopeType, gapType, discoveryPrincipal)` — sample outputs

```text
KeyVault / NoReadPermission     → "Grant Azure 'Reader' to discovery identity {discoveryPrincipal}
                                   at the required subscription, resource-group, or vault scope. Run:
                                     az role assignment create --assignee {discoveryPrincipal} \
                                       --role 'Reader' --scope {scopeId}"
...
AzureSubscription / NoListPermission → "Grant 'Reader' to discovery identity {discoveryPrincipal} on subscription {scopeName}
                                        (id: {scopeId}) or on the parent management group."

EntraTenant / NoListPermission  → "Grant Microsoft Graph delegated permission 'Application.Read.All' to the
                                   connector application, provide tenant admin consent, and recreate the
                                   connection as {discoveryPrincipal}."

DataverseEnvironment / NoReadPermission → "Grant {discoveryPrincipal} the required CLM Dataverse role in
...
},
{
  ...
  "actions": {
    ...
    "Compose_resolution_hint": {
      "type": "Compose",
      "inputs": "@concat('Grant Azure Reader to discovery identity ', variables('discoveryPrincipal'), ' at the required scope for vault ', items('Apply_to_each_vault')?['name'], ' (', items('Apply_to_each_vault')?['id'], ').')"
    },
    "Upsert_coverage_gap": {
      "type": "OpenApiConnection",
      "inputs": {

...
The current `CLMDiscoveryFlow 1.1.0.0` implements credential, application-owner, source-environment, coverage-gap, and renewal-event writes, but it does **not** yet create or complete `clm_discoveryrun` records. Treat the run-record steps below as the required next revision rather than current behaviour.
